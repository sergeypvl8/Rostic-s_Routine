...
def create_task_keyboard(tasks, time_of_day, chat_id):
    """Создает клавиатуру с кнопками задач"""
    keyboard = []
    ...
    keyboard.append([InlineKeyboardButton(
        "🔄 Сбросить все", 
        callback_data=f"reset_{time_of_day}"
    )])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/all - все задачи\n"
        "/set_daily - установить ежедневные напоминания\n"
        "/stop_daily - остановить напоминания\n\n"
        "🕗 Напоминания: 8:00 и 16:00 ежедневно"
    )
...
async def task_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("task_"):
        ...
        if chat_id not in user_tasks:
            user_tasks[chat_id] = {}
        if time_of_day not in user_tasks[chat_id]:
            user_tasks[chat_id][time_of_day] = {}
        
        # Переключение статуса
        current = user_tasks[chat_id][time_of_day].get(task_index, False)
        user_tasks[chat_id][time_of_day][task_index] = not current
...
async def send_morning_daily(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.error(f"Ошибка утренних задач: {e}")
...
async def send_afternoon_daily(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.error(f"Ошибка вечерних задач: {e}")
...
async def set_daily_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🕗 Буду присылать задачи в 8:00 и 16:00"
    )

async def stop_daily_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    jobs = (context.job_queue.get_jobs_by_name(f"{chat_id}_morning") + 
...
def main():
    # Создаем Application с минимальными настройками
